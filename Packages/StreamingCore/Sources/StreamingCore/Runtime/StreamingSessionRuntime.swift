...
extension StreamingSession: StreamingRuntimeDelegate {
    ...
    /// Re-establishes WebRTC against the existing provisioned signaling session without restarting the game.
    public func reconnectWebRTC(bridge newBridge: any WebRTCBridge) async {
        guard lifecycle != .idle else { return }
        ...
        newBridge.delegate = model.bridgeDelegate
        await model.runtime.replaceBridge(newBridge)
        model.metricsSupport.replaceBridge(newBridge)
        model.bridgeDelegate.setActiveGeneration(model.runtimeGenerationBox.current() &+ 1)

        lifecycle = .connectingWebRTC
        await model.runtime.reconnectWebRTC()
    }
    ...
    private func updateLifecycleFromRuntime(_ nextLifecycle: StreamLifecycleState) {
        ...
        case .failed:
            stopStatsPolling()
            if disconnectIntent == .reconnectTransition {
                disconnectIntent = .reconnectable
            }
        case .disconnected:
            stopStatsPolling()
