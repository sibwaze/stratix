...
actor StreamingRuntime {
    ...
    private var bridge: any WebRTCBridge
    ...
    func connect(type: StreamKind, targetId: String, msaUserToken: String?) async {
        ...
        do {
            ...
            try await establishWebRTCMediaPath(using: streamSession)
    ...
    func disconnect(preserveSignalingSession: Bool = false) async {
        if preserveSignalingSession {
            await teardownWebRTCTransportOnly()
        } else {
            await teardownActiveConnection()
        }
    }

    func replaceBridge(_ newBridge: any WebRTCBridge) async {
        await bridge.close()
        bridge = newBridge
    }

    /// Re-establishes WebRTC against an existing provisioned signaling session without restarting the game.
    func reconnectWebRTC() async {
        guard let streamSession = session else {
            await notifyLifecycle(.failed(StreamError(code: .signaling, message: "No signaling session available for hot reconnect")))
            return
        }

        do {
            guard try await streamSession.isResumableForWebRTCReconnect() else {
                throw StreamError(code: .signaling, message: "Signaling session is no longer resumable")
            }
            ...
            streamLogger.error("Hot reconnect failed [\(error.code.rawValue, privacy: .public)]: \(error.message, privacy: .public)")
            await notifyLifecycle(.failed(error))
        } catch let error as APIError {
            let streamError = StreamError(code: .signaling, message: error.localizedDescription)
            streamLogger.error("Hot reconnect failed [\(streamError.code.rawValue, privacy: .public)]: \(streamError.message, privacy: .public)")
            await notifyLifecycle(.failed(streamError))
        } catch {
            let streamError = StreamError(code: .unknown, message: error.localizedDescription)
            streamLogger.error("Hot reconnect failed [unknown]: \(streamError.message, privacy: .public)")
            await notifyLifecycle(.failed(streamError))
        }
    }
    ...
    func handleConnectionStateChange(_ state: PeerConnectionState, generation: UInt64) async {
        ...
        switch state {
        case .connected:
            if let streamSession = session {
                await streamSession.restoreStreamingKeepalive()
            }
            await notifyLifecycle(.connected)
    ...
    private func teardownWebRTCTransportOnly() async {
    ...
    private func establishWebRTCMediaPath(using streamSession: StreamSession) async throws {
        await bridge.prepareForConnect()
