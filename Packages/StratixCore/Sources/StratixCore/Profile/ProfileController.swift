...
public final class ProfileController {
    ...
    private func performLoadCurrentUserPresence(force: Bool) async {
        ...
        do {
            let previousPresence = currentUserPresence
            let presence = try await XboxWebPresenceClient(credentials: credentials).getCurrentUserPresence(level: "all")
            guard !isSuspendedForStreaming else { return }
            if shouldPreservePreviousPresence(previous: previousPresence, new: presence) {
                lastCurrentUserPresenceError = nil
                return
            }
    ...
    private func shouldPreservePreviousPresence(
        previous: XboxCurrentUserPresence?,
        new: XboxCurrentUserPresence
    ) -> Bool {
        let degradedState = new.state.localizedCaseInsensitiveContains("offline") || new.state == "Unknown"
        guard !new.isOnline else { return false }
        guard degradedState else { return false }

        if let previous {
            guard previous.isOnline else { return false }
            return previous.activeTitleName != nil || previous.state.localizedCaseInsensitiveContains("online")
        }

        // Ignore bare degraded payloads on first fetch so shell reachability heuristics stay intact.
        let hasCredibleOfflineEvidence = new.lastSeen != nil || !new.devices.isEmpty
        return !hasCredibleOfflineEvidence
    }
