...
extension LibraryController {
    ...
    private func artworkPrefetchDependencies() -> LibraryArtworkPrefetchCoordinator.Dependencies {
        .init(
            ...
            logger: logger,
            isShellReady: { [weak self] in
                guard let shellBootstrapController = self?.shellBootstrapController else { return true }
                if case .ready = shellBootstrapController.phase {
                    return true
                }
                return false
            },
