...
public actor ArtworkPipeline {
    ...
    public func data(for request: ArtworkRequest, mode: ArtworkLoadMode = .defaultLoad) async throws -> ArtworkResponse? {
        ...
        if let existing = inFlight[key] {
            ...
            inFlight[key] = task
            scheduleInFlightCleanup(for: key, task: task)
            logger.debug(
    ...
    private func scheduleInFlightCleanup(
        for key: String,
        task: Task<ArtworkResponse?, Error>
    ) {
        Task {
            _ = await task.result
            finishInFlightRequest(key: key)
        }
    }

    private func finishInFlightRequest(key: String) {
        inFlight[key] = nil
    }

    private func trimRecentPrefetchCacheIfNeeded() {
        guard recentPrefetchByKey.count > 400 else { return }
