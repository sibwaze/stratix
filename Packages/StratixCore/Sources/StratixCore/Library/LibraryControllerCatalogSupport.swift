...
extension LibraryController {
    ...
    nonisolated static func libraryTitleCount(in sections: [CloudLibrarySection]) -> Int {
        ...
        for item in allLibraryItems(from: sections) {
            seenTitleIDs.insert(item.titleId)
        }
        return seenTitleIDs.count
    }
    ...
    nonisolated static func detailMediaRichness(_ detail: CloudLibraryProductDetail) -> Int {
        var hasProductDetailsMedia = false
        var hasProductDetailScreenshots = false
        var hasProductDetailVideoWithThumb = false
        for asset in detail.mediaAssets where asset.source == .productDetails {
            hasProductDetailsMedia = true
            if asset.kind == .image, isLikelyGameplayScreenshotURL(asset.url) {
                hasProductDetailScreenshots = true
            }
            if asset.kind == .video, asset.thumbnailURL != nil {
                hasProductDetailVideoWithThumb = true
            }
        }

        var score = 0
        if hasProductDetailScreenshots { score += 100 }
        if hasProductDetailVideoWithThumb { score += 70 }
        if hasProductDetailsMedia { score += 35 }
