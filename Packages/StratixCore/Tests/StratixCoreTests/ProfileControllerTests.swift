...
struct ProfileControllerTests {
    ...
    @Test
    func loadCurrentUserPresence_preservesHealthyOnlinePresenceAgainstDegradedOfflineResponse() async {
        let controller = ProfileController(
            presenceLoadWorkflow: { controller, force in
                #expect(force)
                if controller.currentUserPresence == nil {
        ...
        await controller.loadCurrentUserPresence(force: true)
        await controller.loadCurrentUserPresence(force: true)

        let snapshot = controller.profileShellSnapshot()
        #expect(snapshot.isOnline)
        #expect(snapshot.activeTitleName == "Halo Infinite")
        #expect(snapshot.presenceState == "Online")
    }

    @Test
    func loadCurrentUserPresence_ignoresBareDegradedFirstFetch() async {
        let controller = ProfileController(
            presenceLoadWorkflow: { controller, force in
                #expect(force)
                controller.setCurrentUserPresence(
    ...
    @Test
    func loadCurrentUserPresence_acceptsCredibleOfflineFirstFetch() async {
        let controller = ProfileController(
            presenceLoadWorkflow: { controller, force in
                #expect(force)
                controller.setCurrentUserPresence(
        ...
        #expect(snapshot.presenceState == "Offline")
        #expect(snapshot.lastSeenTitleName == "Halo Infinite")
    }
