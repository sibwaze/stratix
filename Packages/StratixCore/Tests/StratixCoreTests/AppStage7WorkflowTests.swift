...
struct AppStage7WorkflowTests {
    ...
    func cacheRestoreWorkflow_restoresAllCachesWhenAuthenticated() async {
        ...
        #expect(Set(calls) == ["library:true", "achievements:true", "profile"])
    }

    @Test
    func startupWorkflow_handleOnAppear_runsSessionRestoreBeforeHapticsProbe() async {
        var calls: [String] = []
        ...
        #expect(calls == ["settings", "session", "haptics"])
    }
