...
final class RenderSurfaceCoordinator {
    ...
    func startStream(
        ...
        let activeBridge = await prepareBridge(
    ...
    /// Clears the one-shot exit guard when a new stream surface is presented.
    func resetExitGuard() {
    ...
    func handleDisappear(
        ...
        guard shouldTearDownSession else { return }
    ...
    /// Returns a fresh WebRTC bridge for manual or automatic relaunch.
    func bridgeForRelaunch(
        surfaceModel: StreamSurfaceModel,
        session: (any StreamingSessionFacade)?
    ) async -> WebRTCClientImpl {
        await makeFreshBridge(surfaceModel: surfaceModel, session: session)
    }

    /// Creates a fresh WebRTC bridge for every stream launch or relaunch.
    ...
    /// Discards any closed bridge and creates a new peer connection for relaunch.
    private func makeFreshBridge(
        surfaceModel: StreamSurfaceModel,
        session: (any StreamingSessionFacade)?
    ) async -> WebRTCClientImpl {
        surfaceModel.reset()
        ...
        if let existingBridge = bridge {
            await existingBridge.close()
        }
        let freshBridge = WebRTCClientImpl()
        bridge = freshBridge
        attachVideoTrackHandler(to: session, surfaceModel: surfaceModel)
        return freshBridge
    }
