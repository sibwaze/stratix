...
final class RendererAttachmentCoordinator: NSObject {
    ...
    private var ladderPlanner: RenderLadderPlanner?
    ...
    func clear() {
        ...
        callbacks = .noop
        if let mtkView = (container as? RendererContainerView)?.mtkView {
            mtkView.removeFromSuperview()
            (container as? RendererContainerView)?.mtkView = nil
        }
        metalRenderer = nil
        attachedTrack = nil
        ladderPlanner = nil
    ...
    private func applyConfiguration(_ configuration: Configuration) {
        ...
        wireMetalRendererCallbacks()
    }

    private func ensureLadderPlanner() -> RenderLadderPlanner {
        if let ladderPlanner {
            return ladderPlanner
        }
        let planner = RenderLadderPlanner(
            resolver: UpscaleCapabilityResolver(probe: LiveUpscaleCapabilityProbe())
        )
        ladderPlanner = planner
        return planner
    }

    @discardableResult
    private func ensureMetalRenderer(in container: RendererContainerView) -> MetalVideoRenderer? {
        if let metalRenderer {
            return metalRenderer
        }
        guard let renderer = MetalVideoRenderer(frame: container.bounds) else {
    ...
    private func wireMetalRendererCallbacks() {
        guard let metalRenderer else { return }
        let eventContinuation = callbackEventPump.continuation
    ...
    private func handleCandidateFailure(_ rung: RenderLadderRung, reason: String) {
        let planner = ensureLadderPlanner()
        candidateFailureCounts = planner.failureCountsAfterFailure(
            rung,
            existingFailureCounts: candidateFailureCounts
        )
        let failureCount = candidateFailureCounts[rung, default: 0]
        deadRungs = planner.deadRungs(
        ...
        updateAttachedRendererFamily()
        if deadRungs.contains(rung) {
            let nextCandidate = ladderPlan.flatMap { planner.nextCandidate(in: $0, deadRungs: deadRungs) }
    ...
    private func refreshLadderPlanIfNeeded(resetDeadRungs: Bool) -> Bool {
        ...
        let nextPlan = ensureLadderPlanner().makePlan(
            device: metalDevice,
    ...
    private func attemptNextCandidateIfNeeded() {
        ...
        guard let nextCandidate = ensureLadderPlanner().nextCandidate(in: ladderPlan, deadRungs: deadRungs) else { return }
        ...
        case .metal4FXSpatial, .metalFXSpatial, .passthrough:
            guard
                let container = (self.container as? RendererContainerView)
                    ?? sampleBufferView?.superview as? RendererContainerView,
                let metalRenderer = ensureMetalRenderer(in: container)
            else {
