...
public final class WebRTCClientImpl: NSObject, WebRTCBridge, @unchecked Sendable {
    ...
    var isAudioSessionObserverRegistered = false
#endif
    ...
    /// Creates a lightweight bridge shell; heavy WebRTC runtime setup is deferred to `prepareForConnect()`.
    public override init() {
        super.init()
    }

    /// Initializes SSL, codec factories, and tvOS audio defaults once per process.
...
#if os(tvOS)
        if !isAudioSessionObserverRegistered {
            RTCAudioSession.sharedInstance().add(self)
            isAudioSessionObserverRegistered = true
        }
#endif
    }
    ...
    public func prepareForConnect() async {
