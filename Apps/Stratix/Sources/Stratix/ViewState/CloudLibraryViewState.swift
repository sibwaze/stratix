...
enum CloudLibrarySettingsPane: String, CaseIterable, Identifiable, Hashable, Sendable {
    ...
    /// Maps persisted pane identifiers, including retired values, onto the current pane set.
    static func fromStoredRawValue(_ rawValue: String) -> CloudLibrarySettingsPane? {
        if rawValue == "overview" {
            return .stream
        }
        return CloudLibrarySettingsPane(rawValue: rawValue)
    }
}
