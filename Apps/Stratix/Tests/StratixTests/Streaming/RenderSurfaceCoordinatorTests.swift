...
struct RenderSurfaceCoordinatorTests {
    ...
    @Test
    func bridgeForRelaunchCreatesFreshBridgeInstance() async {
        let coordinator = RenderSurfaceCoordinator()
        let surfaceModel = StreamSurfaceModel()

        let firstBridge = await coordinator.bridgeForRelaunch(surfaceModel: surfaceModel, session: nil)
        let secondBridge = await coordinator.bridgeForRelaunch(surfaceModel: surfaceModel, session: nil)
